...
func (c *DPFMAPICaller) HeaderDoc(
	...
	if input.HeaderDoc.FreightOrder != nil {
		where = fmt.Sprintf("%s\nAND FreightOrder = %d", where, *input.HeaderDoc.FreightOrder)
	}
	...
	groupBy := "\nGROUP BY FreightOrder, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    FreightOrder, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_freight_order_header_doc_data
		` + where + groupBy + `;`)
	if err != nil {
